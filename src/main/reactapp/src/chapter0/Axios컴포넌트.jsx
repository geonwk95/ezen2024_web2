...
export default function Axios컴포넌트(props){
    ...
    const doGet = async () => {
        ...
        console.log( 3 );
        axios.get("https://jsonplaceholder.typicode.com/comments?postId=1") // 쿼리스트링
        .then( response => { console.log(response); })
        .catch( error => { console.log( error ); })
    ...
    const doPost = () => {
        ...
        axios.post("http://localhost:8080" , axiosFormData )
        .then( response  => { console.log( response )})
        .catch( error => { console.log( error ); })
    }
    // 3. PUT
    ...
    return(<>
        ...
        <button type="button" onClick={ doGet }> doGet Axios실행 </button>
        <form id="axiosForm">
            <input type="text" />
        </form>
        <button type="button" onClick={ doPost }> doPost Axios실행 </button>
        <button type="button" onClick={ doPut }> doPut Axios실행 </button>
        <button type="button" onClick={ doDelete }> doDelete Axios실행 </button>
    </>)
}